	...
	for (int value = 1; value <= capacity; ++value) {
		...
		if (total <= MAX_BLOCKERS && pairs <= MAX_PAIR_BLOCKERS) {
			...
			for (int row = 0; row < ROWS; ++row) {
				...
				bool has_later_incidence = false;
				for (int later = position + 1; later < static_cast<int>(active_supports.size()); ++later) {
					if (active_supports[later] & (1 << row)) has_later_incidence = true;
				}
				if (!has_later_incidence) can_reach = false;
			}
			if (can_reach) {
...
int main() {
	for (int support = 1; support < FULL; ++support) {
		if (__builtin_popcount(static_cast<unsigned>(support)) <= 3) {
	...
	for (std::uint16_t active = 1; active < limit; ++active) {
		...
		for (int index = 0; index < static_cast<int>(covers.size()); ++index) {
			...
			if (active_covers.size() > MAX_BLOCKERS || pair_covers > MAX_PAIR_BLOCKERS) break;
		}
	...
	for (const auto& [key, count] : blocker_histogram) {
		std::cout << "blockers=" << key.first << " pairs=" << key.second
			<< " vectors=" << count << '\n';
	}
