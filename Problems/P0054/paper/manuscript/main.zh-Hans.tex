...
\date{内部审阅中文对照稿，2026 年 7 月 30 日}
...
\begin{theorem}[小于十六的中间 blocker 分类]
\label{thm:m33}
精确局部参数为 $m(3,3)=14$。总基数为 14 的任意合格 pair，在删除非活跃点并
重标活跃底集后，都是 Fano 直线 clutter 与自身组成的 pair。不存在总基数为
15 的合格 pair。
\end{theorem}

\begin{proof}
补充材料 S2--S3 节给出任意有限证明。私有见证把每行大小限制在对侧行数以内；
...
七条 Fano 直线构成自 blocker clutter，达到总数 14。等号分类用同样的残余与
支撑归约排除非对称拆分；中央 $(7,7)$ 拆分具有 Fano 关联模式，blocker 对合
固定另一侧。加入一个松弛行后重复归约，可排除全部总数 15 拆分。补充材料列出
每个有限域及独立重放；对应 Lean 定理量化原始的任意有限底集类型。
\end{proof}
...
或使中间 blocker 对的总数为 15；后者由\cref{thm:m33}排除。所以总数 53
也不可能。
...
证明没有限制原始迹底集的大小。补充材料 S5--S10 节证明这些归约，并说明有限
输出如何推出任意底集结论。C++ 枚举器与独立 Python 实现逐字节重放保存输出；
Lean 证明把可执行检查连接到原始 clutter 的可靠性与实现引理。因此四个拆分及
其 blocker 对偶均不可能。被强制存在的三元行无处可放，从而排除 $P_{16}$。
\end{proof}
...
完整精确值链于 2026 年 7 月 27 日首次闭环，并在本轮修改后于 7 月 30 日重新
通过最终门。整个 \texttt{AiMathLab} 库共处理 17488 个构建任务；
