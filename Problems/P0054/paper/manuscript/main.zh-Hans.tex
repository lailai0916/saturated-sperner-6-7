...
\hypersetup{
  ...
  pdftitle={饱和6-Sperner数与7-Sperner数的精确值},
...
\author{Jiazhi Chen\\
\small 浙江省杭州第二中学\\
\small 中国浙江省杭州市滨江区东信大道 76 号，310053\\
...
\date{内部审阅中文对照稿，2026 年 7 月 29 日}
...
\begin{abstract}
本文确定最终稳定的饱和 6-Sperner 数与 7-Sperner 数。六层情形的精确值为
$\sat(6)=30$。下界归结为两个中间 canonical 层对应的有限 clutter 参数
$m(2,3)$；本文对任意有限底集证明 $m(2,3)=9$。七层情形中，一个更强的
blocker 不等式先给出 $m(2,4)=12$ 与 $\sat(7)\geq47$。连续的任意有限归约
...
2026 年 7 月 29 日完成的投稿前终检覆盖 Crossref、arXiv、OpenAlex、
Google Scholar、已索引引用链及直接相关全文。在记录范围内，检索没有找到
两个精确值的公开等价结果。这项有限负面检索不能证明新颖性或优先权。
...
下面的引理给出 canonical 层与 blocker 之间的接口。

\begin{lemma}[Canonical blocker 接口]
\label{lem:canonical-blocker}
设 $\mathcal A_i$ 是一个极小饱和 $k$-Sperner 族的内部 canonical 层，
$H$ 为其齐次原子，并令 $U=X\setminus H$。令 $\mathcal S_i$ 为
$\mathcal A_i$ 的小成员。把每个大成员写成 $K\cup H$，其中 $K\subseteq U$，
再定义
\[
...
\begin{proof}
大小结论来自 Martin--Veldt 的命题 14~\cite{martinveldt2025}。取非空真子集
$Q\subset H$。对每个 $R\subseteq U$，混合集 $R\cup Q$ 不属于该层。
$\mathcal A_i$ 的饱和性说明：存在 $S\in\mathcal S_i$ 使 $S\subseteq R$，
或者存在大迹 $K$ 使 $R\subseteq K$。反链性质说明，每个 $S$ 都与每个
$U\setminus K$ 相交。

若 $T$ 是 $\mathcal C_i$ 的横截集，则 $T$ 不包含于任何大迹。上述覆盖条件
给出某个 $S\subseteq T$。所以极小横截集恰为 $\mathcal S_i$ 的成员。对
$U\setminus T$ 使用同一论证得到反向 blocker 等式；也可由
\cref{lem:blocker-basic}和两侧的 clutter 性直接得到。
\end{proof}
...
在稳定区间内选取满足 $n\geq8$ 且 $n>2^{30}$ 的底集，并在其上选取端点正规
的极小族 $\family$。由于 $|\family|\leq30$，齐次原子定理适用。Canonical
分解给出六个非空饱和反链层 $\mathcal A_0,\ldots,\mathcal A_5$。端点结论与
Martin--Veldt 的引理 15~\cite{martinveldt2025}给出
\[
  |\mathcal A_0|=|\mathcal A_5|=1,\qquad
  |\mathcal A_1|,|\mathcal A_4|\geq5.
\]
由\cref{lem:canonical-blocker}，第二层构成满足条件的 $(2,3)$ blocker pair；
...
\begin{proof}
使用\cref{lem:canonical-blocker}的记号。$\mathcal S_3$ 与
...
\begin{proof}
选取满足 $n>2^{55}$ 的稳定底集。\cref{thm:size55}的构造说明极小族至多有
55 个成员，所以齐次原子与 canonical 分解均存在。端点和相邻层下界给出
\[
  (|\mathcal A_0|,|\mathcal A_1|,|\mathcal A_5|,
  |\mathcal A_6|)\geq(1,6,6,1).
\]
Canonical blocker 接口与\cref{thm:m24}给出
...
记 $\mathcal C_i$ 为大迹的补族，定义同\cref{lem:canonical-blocker}。
...
\begin{proof}
端点、相邻层与 blocker 下界给出
\[
  (|\mathcal A_0|,\ldots,|\mathcal A_6|)
  \geq(1,6,12,14,12,6,1).
  \tag{2}
\]
中间层下界来自私有见证、残余 blocker 和三行关联公式。证明检查 blocker 总数
不超过 13 时的每个拆分。若每个点的度至多为二，则活跃点构成固定行标签上的实际
自环多重图。包含极小性迫使每个正支撑接触一个最小度行；行大小界把问题归结为
有限且穷尽的支撑分类。两个独立实现对全部保存分类给出相同结果。

式 (2) 取等时，中间 blocker 对只能是 Fano 平面。它无法与下方或上方的 12 元
等号相邻层兼容，所以总数 52 不可能。若仅增加一个成员，则仍落入 Fano 排除，
或使中间 blocker 对的总数为 15。拆分 $(5,10)$、$(6,9)$、$(7,8)$ 及其对偶
均被同一类任意有限度数与残余归约排除，所以总数 53 也不可能。
...
$D\subseteq R$ 与 $E\subseteq U\setminus R$ 构成穷尽且不交的覆盖。若 16 行
大小都至少为四，则并合界必须取等；但同一侧的两个事件以正概率相交，产生矛盾。
所以至少有一条中间行大小为三。
...
证明没有限制原始迹底集的大小。C++ 枚举器与独立 Python 实现逐字节重放保存的
输出。因此四个拆分及其 blocker 对偶均不可能。被强制存在的三元行无处可放，
从而排除 $P_{16}$。
\end{proof}
...
\begin{proof}
在稳定范围内选择一个极小族。55 元构造保证齐次原子归约可用。52 元与 53 元
排除给出 $\sat(7)\geq54$，而\cref{prop:exclude54}排除等号。因此
...
具体的十一元素实现还使用 Mathlib~\cite{mathlib2020} 在 Lean 4
\cite{lean4} 中完成了形式化。Lean 证明这个显式族的基数为 55，是
...
G4.10、G4.11、G4.12 清单分别绑定 8、23、36 个证明、源文件和输出对象。全部保存
枚举均已重放，独立实现对语义计数给出一致结果。有限计算仅在所展示的任意有限
归约之后成为证明依赖。

Lean 形式化首先覆盖了 $(8,8)$ 分支中的局部五行三度核；定理
...
2026 年 7 月 27 日的最终门完成了整个 \texttt{AiMathLab} 库的构建，共处理
17488 个构建任务；\path{Problems/P0054/formal/Main.lean} 的独立检查也成功。
...
Lean 内核验收不能替代对任意有限底集归约和证书接口的人类重构与审查。投稿前
终检在记录范围内没有找到公开等价结果；该负面结果仍然有限，不能证明新颖性或
优先权。

另一个后续问题是猜想 $\sat(k)=\operatorname{A075529}(k)$，其中
\texttt{A075529} 记录星型加法链终值的不同可能数目。该猜想目前标为
\texttt{UNKNOWN}，本文既未证明也未证伪。前七项的数值吻合不构成通项公式的
结构性证据；相关研究留待后续工作。
...
依赖。该工件包已按版本化 GitHub 发布和不可变 Zenodo 归档的要求整理；正式
投稿前将在此补入公开发布地址和 DOI。

\par\medskip\noindent\textbf{生成式 AI 与 AI 辅助技术声明。}\quad
在准备本文期间，作者使用 OpenAI Codex 辅助文献检索式设计、证明与代码起草、
验证编排和语言编辑。作者审阅并修改了所有生成内容，并对论文全部内容承担责任。
数学主张依赖文中展示的论证、可独立重放的证书、直接验证器和 Lean 内核的接受，
而不依赖模型输出。
