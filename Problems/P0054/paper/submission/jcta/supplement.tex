...
\hypersetup{
  pdftitle={Supplementary proofs and verification for The exact saturated 6- and 7-Sperner numbers},
...
\title{Supplementary proofs and verification for\\
...
\begin{abstract}
This supplement contains the arbitrary-finite reductions used to prove
$\sat(6)=30$ and $\sat(7)=55$.  It gives the full middle-blocker
classifications, including the Fano equality case and the four splits of total
16.  The support-quotient lemma retains inactive points, parallel points,
repeated incidence patterns, nonuniform rows, and empty optional classes.  A
completeness theorem then expands each finite support back to the original
ground set.  The final sections describe the corresponding Lean declarations
and the independent computational checks.
\end{abstract}

\tableofcontents

\section{Blocker preliminaries and evidence conventions}
\label{sec:evidence}
...
\section{Completeness of the finite reductions}
\label{sec:finite-interface}

This section isolates, as a standalone mathematical statement, the interface
common to every enumeration above.  Suppose a residual clutter
$\mathcal H=\{H_1,\ldots,H_r\}$ has labelled rows on a finite ground set
$U$, with every $H_i$ nonempty.  Put $U^*=\bigcup_iH_i$ and map each active
actual point $u\in U^*$ to its nonempty incidence support
\[
 I(u)=\{i\in[r]:u\text{ belongs to row }i\}.                \tag{S10.1}
\]
For every nonempty $I\subseteq[r]$, let
$P_I=\{u\in U^*:I(u)=I\}$ and $m_I=|P_I|$.  Thus points with the same support
are counted, not identified.  A \emph{support cover} is a set $M$ of
nonempty subsets of $[r]$ whose union is $[r]$; it is minimal if none of its
proper subfamilies is a support cover.

\begin{lemma}[Support quotient and exact expansion]
\label{lem:support-quotient}
There is a bijection
\[
 \blocker(\mathcal H)
 \longleftrightarrow
 \coprod_{\substack{M\text{ a minimal support cover}\\m_I>0\ (I\in M)}}
 \prod_{I\in M}P_I.                                      \tag{S10.2}
\]
Under this bijection a transversal $T$ uses exactly one actual point from
each support class indexed by $M$.  In particular,
\[
 |\blocker(\mathcal H)|
   =\sum_M\prod_{I\in M}m_I,
 \qquad
 \min_{T\in\blocker(\mathcal H)}|T|=\min_M|M|,           \tag{S10.3}
\]
and
\[
 |H_i|=\sum_{I\ni i}m_I,
 \qquad
 \deg_{\mathcal H}(u)=|I(u)|.                            \tag{S10.4}
\]

Conversely, from a vector $(m_I)$ construct the labelled expansion
\[
 U(m)=\{(I,j):\varnothing\ne I\subseteq[r],\ 1\leq j\leq m_I\},
 \qquad
 H_i(m)=\{(I,j):i\in I\}.                                \tag{S10.5}
\]
After deleting inactive points, the map sending the $j$th member of $P_I$
to $(I,j)$ is an isomorphism from $\mathcal H$ to
$\mathcal H(m)=\{H_i(m):i\in[r]\}$.  It induces an isomorphism of their
blockers.  Hence expansion preserves row sizes, transversal sizes, point
degrees, blocker cardinality, and blocker equality exactly.
\end{lemma}

\begin{proof}
If a minimal transversal $T$ contained distinct $x,y\in P_I$, then deleting
$y$ would not change the set of rows met by $T$, contradicting minimality.
Thus $T$ selects at most one representative of each support.  The supports
it selects cover $[r]$ because $T$ meets every row.  If that support cover
were not minimal, deleting the representative of a redundant support would
again leave a transversal.  This maps $T$ to the right-hand side of
(S10.2).

Conversely, let $M$ be a minimal support cover and choose one point from each
$P_I$, $I\in M$.  The chosen points meet every row.  For each $I\in M$,
minimality of $M$ supplies an index in $I$ that belongs to no other support
in $M$; deleting the representative of $I$ therefore misses that row.  The
chosen set is a minimal transversal.  The two constructions are inverse.
Equations (S10.3)--(S10.4) follow immediately.  The expansion (S10.5)
retains one labelled point for every member of every $P_I$, so the displayed
point bijection preserves membership in every row.  Applying it to minimal
transversals proves the blocker assertion.
\end{proof}

\begin{theorem}[Completeness of the finite classification reductions]
\label{thm:finite-classification-completeness}
Let $(\mathcal S,\mathcal C)$ be a mutual-blocker pair on an arbitrary finite
ground set, with $|\mathcal S|=r$ and $|\mathcal C|=s$.  After deleting
inactive points and labelling the rows of $\mathcal S$, there is a vector
\[
 (m_I)_{\varnothing\ne I\subseteq[r]},
 \qquad 0\leq m_I\leq s,                                 \tag{S10.6}
\]
whose expansion is isomorphic to $\mathcal S$ and whose blocker is
isomorphic to $\mathcal C$.  The vector preserves the two minimum row sizes,
all point degrees, all row cardinalities, and every minimum-transversal and
blocker-cardinality condition used in Sections S2--S9.

Conversely, if a vector satisfying (S10.6) expands to a clutter
$\mathcal H(m)$ with $|\blocker(\mathcal H(m))|=s$, then
$(\mathcal H(m),\blocker(\mathcal H(m)))$ is a mutual-blocker pair; any
listed row-size, degree, or transversal conditions verified after expansion
hold for that pair.  Therefore exhausting all such vectors, after imposing
only branch conditions already proved necessary, exhausts the original
arbitrary-finite branch.  The same statement applies after exchanging the
two blocker sides and at each residual or row-extension step.
\end{theorem}

\begin{proof}
Apply \cref{lem:support-quotient} to $\mathcal S$.  Because each row of
$\mathcal S=\blocker(\mathcal C)$ is a minimal transversal of the $s$ rows
of $\mathcal C$, \cref{lem:private} gives $|S|\leq s$ for every
$S\in\mathcal S$.  If $m_I>0$ and $i\in I$, then
$m_I\leq|S_i|\leq s$, proving (S10.6).  The expansion recovers
$\mathcal S$ exactly up to relabelling, and the blocker part of
\cref{lem:support-quotient} recovers $\mathcal C$.  Equations
(S10.3)--(S10.4) prove preservation of all listed invariants.

For the converse, blocker involution for finite clutters gives
$\blocker(\blocker(\mathcal H(m)))=\mathcal H(m)$.  Thus the expanded pair
is mutual-blocking.  There are only $2^r-1$ coordinates in (S10.6), each in a
finite range.  Clutter minimality and all additional conditions are checked
on the exact expansion, so filtering cannot create a spurious pair.  In a
residual or extension branch, the proved row-count and private-witness bounds
again provide the finite ranges; applying the same argument to the enlarged
labelled row family proves the final sentence.
\end{proof}

This theorem is the logical bridge from the arbitrary input pair to each
finite kernel.  Rank bounds, no-full-support conclusions, point-degree
bounds, and forced-incidence rules in Sections S2--S9 are branch lemmas that
only shrink (S10.6).  Extension checks include every permitted incidence of
an existing point and every nonempty support of a new point.  The semantic
verifiers then expand all labelled copies and recompute both blockers.
Consequently inactive points are harmless; multiplicity $m_I>1$ retains
parallel actual points and repeated incidence patterns; zero coordinates
retain empty optional classes; and no uniformity of row sizes is assumed.
...
\end{center}
This declaration constructs the interface (S10.7)--(S10.9) from the natural
...
about arbitrary finite clutters follows only after
\cref{lem:support-quotient,thm:finite-classification-completeness} and the
branch-specific necessity lemmas.  In the final artifact the branchwise
support/realization interfaces and the classifications are also checked in
Lean.
...
Support quotient and inverse expansion
& \Cref{lem:support-quotient}; equations (S10.2)--(S10.5)
& Worked formal instance:
  \path{G413.threeK2_actual_support_classification};
  \path{G413.finiteBlocker_eq_blocker} \\
Completeness of finite classification reductions
& \Cref{thm:finite-classification-completeness}; branch domains in
  Sections S5--S9
& Realized branchwise by the soundness, actual-support, and closing
  declarations in the rows below \\
...
{GitHub release v1.1.2} and
\href{https://doi.org/10.5281/zenodo.21770438}{Zenodo DOI
10.5281/zenodo.21770438}.  These records identify the formal-source,
certificate, and replay snapshot used here.  The present manuscript-only
clarifications postdate that immutable archive and do not change any Lean
source, certificate, manifest, or executable result.
...
\end{verbatim}
The pinned formal environment is:
\begin{center}
\small
\begin{tabular}{@{}ll@{}}
...
\end{tabular}
\end{center}
These identifiers are read from \path{lean-toolchain} and
\path{lake-manifest.json}, rather than from an unversioned global
installation.  The locked supporting environment
requires Python 3.12; the reference replay for this revision used Python
3.12.13 under uv 0.11.29.  The local source-replay environment reports Apple
clang 21.0.0; the experiment reports preserve the exact C++17 or C++20 flags
for each enumerator.  Compiler and interpreter versions affect
reproducibility, but not the Lean kernel's acceptance of the exported
theorems.
