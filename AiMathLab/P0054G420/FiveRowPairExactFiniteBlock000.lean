...
theorem four_exceptional_pair_exact_block_000 (offset : Fin 64) :
    smallFourClassificationMaskCheck 3
      fourExceptionalMultiplicityLeafValid
      fourExceptionalMultiplicityInCertificate (0 + offset.val) = true := by
  fin_cases offset
  · rfl
  · rfl
  · rfl
...
theorem three_padded_pair_exact_block_000 (offset : Fin 64) :
    smallFourClassificationMaskCheck 0
      threePaddedMultiplicityLeafValid
      threePaddedMultiplicityInCertificate (0 + offset.val) = true := by
  fin_cases offset
  · rfl
  · rfl
  · rfl
