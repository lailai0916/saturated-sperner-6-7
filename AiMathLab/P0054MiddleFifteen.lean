...
import AiMathLab.P0054MiddleSixNine
import AiMathLab.P0054MiddleSevenEight

/-!
# Exclusion of a fifteen-member middle blocker pair
-/

namespace AiMathLab.P0054.MiddleFifteen

open AiMathLab.P0054.G413

theorem middle_fifteen_impossible {α : Type*}
    [Fintype α] [DecidableEq α]
    {H : G413.Hypergraph α}
    (hHClutter : IsClutter H)
    (hTotal : H.card + (blocker H).card = 15)
  ...
  have hHLower : 3 ≤ H.card := by
    ...
    have hTUpper := minimalTransversal_card_le_rows (mem_blocker.mp hTB)
    exact (hBRows T hTB).trans hTUpper
  ...
  have oriented_impossible {K : G413.Hypergraph α}
      ...
      (hKTotal : K.card + (blocker K).card = 15)
    ...
    · have hBCard : (blocker K).card = 9 := by omega
      exact AiMathLab.P0054.MiddleSixNine.sixNine_impossible
        hKClutter hCard hKRows hBKRows hBCard
    · have hBCard : (blocker K).card = 8 := by omega
      exact AiMathLab.P0054.MiddleSevenEight.sevenEight_impossible
        hKClutter hCard hKRows hBKRows hBCard
  ...
  · have hDualClutter : IsClutter (blocker H) := blocker_isClutter H
    have hDualTotal :
        (blocker H).card + (blocker (blocker H)).card = 15 := by
      rw [blocker_involution hHClutter]
      omega
...
end AiMathLab.P0054.MiddleFifteen
